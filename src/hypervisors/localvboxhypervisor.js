...
var IHypervisor = function() {
    ...
    var name = "localvboxhypervisor.js";
	...
	this.isRunning = function(vmname, callback) {
		...
		var cmd = 'vboxmanage list runningvms > ' + filename;
	...
	this.restoreVM = function(vmname, snapshotname, callback) {
		...
		// Define virtualbox command.
		var cmd = 'vboxmanage snapshot ' + vmname + ' restore ' + snapshotname;
		tr.log(5,m,'Running: ' + cmd);
	...
	this.startVM = function(vmname, snapshotname, callback) {
		...
		// Define virtualbox command.
		var cmd = 'vboxmanage startvm ' + vmname + ' --type headless';
		tr.log(5,m,'Running: ' + cmd);
	...
	this.stopVM = function(vmname, callback) {
		...
		// Define virtualbox command.
		var cmd = 'vboxmanage controlvm ' + vmname + ' savestate';
		tr.log(5,m,'Running: ' + cmd);
	...
	this.getIP = function(vmname, callback) {
		...
		// DNS Lookup
		dns.lookup(vmname, function(err, address, family) {
			var m = "getIPCallback";
			if (err) {
				...
				var msg = "Returning IP for VM " + vmname + " ip=" + address;
				tr.log(5,m,msg);
	...
	this.takeSnapshot = function(vmname, snapshotname, callback) {
		...
		var cmd = 'vboxmanage snapshot ' + vmname + ' take ' + snapshotname;
		tr.log(5,m,'Running: ' + cmd);
	...
	this.renameSnapshot = function(vmname, srcsnapshotname, dstsnapshotname, callback) {
		...
		// Define virtualbox command.
		// FUTURE TODO: Ensure the VM is stopped before proceeding.
		var cmd = 'vboxmanage snapshot ' + vmname + ' edit ' + srcsnapshotname + ' --name ' + dstsnapshotname;
		tr.log(5,m,'Running: ' + cmd);
